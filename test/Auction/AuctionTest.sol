...
contract auctionTest is Test {
    ...
    function testEndAuctionNoBidds() public {
        // the seller is highest bid
        vm.startPrank(seller);
        vm.warp(block.timestamp + 7 days);
        vm.expectRevert();
        auction.endAuction();
    ...
    function testNotEndAuction() public {
    ...
    function testEndAuctionNotExist() public {
        // user read function is not the seller
        vm.warp(block.timestamp + 7 days);
        vm.expectRevert();
        auction.endAuction();
    }
