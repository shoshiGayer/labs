...
contract LendingProtocol is  Ownable, MyMath {
...
//check burn msg.sender
        ...
        returns (uint256, uint256)
    {
        ...
        (,uint256 paid )= _amount.trySub(fee);
        return (fee, paid);
    }
    ...
    function liquidation(address _user) external onlyOwner {
        ...
        if (borrowed > percentage(collateralToUsd, maxLTV)) {
            ...
            if (amountDai > borrowed) {
              ...
              (, uint256 extraAmount) = amountDai.trySub(borrowed);
    ...
    function harvestRewards() external onlyOwner {
        ...
        if (aWethBalance > totalCollateral) {
           (, uint256 rewards )= aWethBalance.trySub(totalCollateral);
    ...
    function getExchangeRate() public view returns (uint256) {
        ...
        (, uint256 num) = cash.tryAdd(totalBorrowed);
        (, uint256 num2)= cash.tryAdd(totalReserve);
        (, uint256 num3)= num.tryAdd(num2);
        return getExp(num3, bondToken.totalSupply());
    }

    function getCash() public view returns (uint256) {
        (, uint256 cash) = totalDeposit.trySub(totalBorrowed);
        return cash;
    }
    ...
    function _interestMultiplier() public view returns (uint256) {
        ...
        (,uint256 num) = fixedAnnuBorrowRate.trySub(baseRate);
    ...
    function _borrowRate() public view returns (uint256) {
        ...
        (, uint256 borrowRate)=product.tryAdd(baseRate);
        return borrowRate;
    }
