...
contract Wallet {
    ...
    modifier onlyOwner() {
        require(owner == msg.sender, "Only the owner and the gabaim allowed to withdraw Ether");
        _;
    }
    ...
    function withdraw(uint256 wad) external {
        require(
            owner == msg.sender || gabaim[msg.sender] == 1, "Only the owner and the gabaim allowed to withdraw Ether"
        );
